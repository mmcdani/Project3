...
```{r}
saveRDS(diabetes_data, "data/diabetes_cleaned.rds")
```
...
Is diabetes more prevalent depending on gender
```{r}
ggplot(
...
ggplot(
  ...
  aes(x = Smoker, fill = Diabetes_binary)
) +
  geom_bar(position = position_dodge(width = 0.9)) +
  geom_text(
    aes(label = after_stat(count)),
...
ggplot(
  ...
  scale_fill_discrete()
```
**double check this people with high alcohol consumption are more likely to have diabetes than people without high alcohol consumption**

physical activity  
```{r}
ggplot(
  data = diabetes_data |> drop_na(PhysActivity, Diabetes_binary),
  aes(x = PhysActivity, fill = Diabetes_binary)
) +
  geom_bar(position = position_dodge(width = 0.9)) +
  geom_text(
    aes(label = after_stat(count)),
  ...
  scale_fill_discrete()
```
**double check this - people that exercise are almost twice as likely to have diabetes** 

Age
```{r}
ggplot(
  data = diabetes_data |> drop_na(Age, Diabetes_binary),
  ...
  scale_fill_discrete()
```
Between the ages of 50  and 80 and older seem to be more likely to have diabetes. 

High Blood Pressure
```{r}
ggplot(
  data = diabetes_data |> drop_na(HighBP, Diabetes_binary),
  aes(x = HighBP, fill = Diabetes_binary)
) +
  geom_bar(position = position_dodge(width = 0.9)) +
  geom_text(
    aes(label = after_stat(count)),
...
Participants with high blood pressure are 4 times as likely to have diabetes than participants withough high blood pressure. 
High Cholesterol
```{r}
ggplot(
  data = diabetes_data |> drop_na(HighChol, Diabetes_binary),
  aes(x = HighChol, fill = Diabetes_binary)
) +
  geom_bar(position = position_dodge(width = 0.9)) +
  geom_text(
    aes(label = after_stat(count)),
...
From the study the participants with high cholesterol seem to be twice as likely to have diabetes than participants without high cholesterol. 
